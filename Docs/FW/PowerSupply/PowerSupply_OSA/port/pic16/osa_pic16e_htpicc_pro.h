...
#define _OS_SET_FSR_CUR_TASK()      FSR0L = (OS_FSR_TYPE)_OS_CurTask; FSR0H = 0;
#define _OS_SET_FSR_CUR_TASK_W(w)   {FSR0L = (OS_FSR_TYPE)_OS_CurTask; FSR0H = 0; asm(" dw 0x3100 +"#w);}
...
#define OS_Task_Replace(priority, TaskName)                                     \
    {                                                                           \
        ...
        INDF0 = priority | 0x48; /* bEnable è bReady */                         \
...
#define     OS_RI(cSavedInt)    OSM_BEGIN {    if (cSavedInt & 0x80) GIE = 1; } OSM_END
#define     OS_EI()             OSM_BEGIN {    GIE = 1; } OSM_END
